...
async function applyWeightReduction(item, containerItem, context = {}) {
    ...
    console.log(`${MODULE_ID} | >>>>> APPLY WEIGHT REDUCTION START for ${item.name} (${item.id}) | Reduction: ${reductionPercent}% <<<<<`); // Яркий лог
    ...
    if (baseWeightForCalc === 0) { // Не трогаем предметы с нулевым весом, кроме флага
        ...
        console.log(`${MODULE_ID} | applyWeightReduction: Base weight is 0. Skipping actual reduction for ${item.name}.`);
        return;
    }

    const reductionFactor = 1 - (reductionPercent / 100);
    const expectedReducedWeight = Math.max(0, baseWeightForCalc * reductionFactor);
    console.log(`${MODULE_ID} | applyWeightReduction: Expected Reduced Weight = ${expectedReducedWeight.toFixed(5)}`); // Точность
    ...
    // Вес нужно обновить, если текущий вес отличается от ожидаемого (с погрешностью)
    ...
    if (needsUpdate) {
        ...
        console.log(`${MODULE_ID} | applyWeightReduction: Weight update finished for ${item.name}.`);
    } else if (needsFlag) { console.log(`${MODULE_ID} | applyWeightReduction: Only flag update was needed for ${item.name}.`); }
     console.log(`${MODULE_ID} | >>>>> APPLY WEIGHT REDUCTION END for ${item.name} <<<<<`); // Конец
}
...
async function restoreOriginalWeight(item, context = {}) {
    ...
    console.log(`${MODULE_ID} | >>>>> RESTORE ORIGINAL WEIGHT START for ${item.name} (${item.id}). Flag value: ${originalWeight} <<<<<`);
    ...
    console.log(`${MODULE_ID} | >>>>> RESTORE ORIGINAL WEIGHT END for ${item.name} <<<<<`);
}
...
Hooks.on('renderItemSheet5e', (app, html, data) => {
    ...
    // --- УЛУЧШЕННАЯ ЛОГИКА ВНЕДРЕНИЯ HTML ---
    const weightlessCheckbox = html.find('input[name="system.properties.weightlessContents"]');
    let targetElement = weightlessCheckbox.closest('.form-group');

    if (targetElement.length > 0) {
         console.log(`${MODULE_ID} | Found 'Weightless Contents' form-group. Inserting controls AFTER it.`);
         html.find('.cwm-reduction-controls').remove();
         targetElement.after(controlsContainerHtml);
    } else {
        console.warn(`${MODULE_ID} | 'Weightless Contents' form-group not found. Trying 'Container Details' fieldset.`);
        const containerDetailsHeaderText = game.i18n.localize("DND5E.ContainerDetails");
        targetElement = html.find(`legend:contains("${containerDetailsHeaderText}")`).closest('fieldset');
        if (targetElement.length > 0) {
...
async function checkCapacityOnCreate(wrapped, embeddedName, data, context) {
    if (embeddedName !== 'Item' || !Array.isArray(data) || data.length === 0) { return wrapped(embeddedName, data, context); } const actor = this; if (!actor || !(actor instanceof Actor)) { return wrapped(embeddedName, data, context); }
    console.log(`${MODULE_ID} | >>>>> CHECK CAPACITY ON CREATE START | Actor: ${actor.name} | Items: ${data.length} <<<<<`); // Яркий лог
    const itemsToCreate = []; let capacityBlocked = false;
    for (const itemData of data) { const currentItemData = foundry.utils.deepClone(itemData); console.log(`${MODULE_ID} | checkCapacityOnCreate: Processing item data:`, currentItemData); if (!currentItemData?.system || !currentItemData.name || ['container', 'backpack'].includes(currentItemData.type)) { console.log(`${MODULE_ID} | checkCapacityOnCreate: Skipping check for item (invalid data or container type): ${currentItemData.name}`); itemsToCreate.push(currentItemData); continue; } const containerId = currentItemData.system.container; if (!containerId) { itemsToCreate.push(currentItemData); continue; } const containerItem = actor.items.get(containerId); const weightCapacityValue = containerItem?.system?.capacity?.weight?.value; const containerHasWeightCapacity = weightCapacityValue !== null && weightCapacityValue !== undefined && weightCapacityValue !== "" && Number.isFinite(Number(weightCapacityValue)); if (!containerHasWeightCapacity) { itemsToCreate.push(currentItemData); continue; } console.log(`${MODULE_ID} | checkCapacityOnCreate: Checking item ${currentItemData.name} against container ${containerItem.name}`); const containerMaxWeight = Number(weightCapacityValue) ?? 0; const currentWeight = calculateContainerContentsWeight(containerItem, actor); let itemBaseWeight = currentItemData.system.weight ?? 0; const quantity = currentItemData.system.quantity ?? 1; const reductionPercent = getReductionPercent(containerItem); let reducedWeight = itemBaseWeight; let needsModification = false; if (reductionPercent > 0 && itemBaseWeight > 0) { reducedWeight = Math.max(0, itemBaseWeight * (1 - reductionPercent / 100)); if(Math.abs(reducedWeight - itemBaseWeight) > 0.001) { needsModification = true; } } const addedWeight = reducedWeight * quantity; console.log(`${MODULE_ID} | checkCapacityOnCreate: Container=${containerItem.name}, Item=${currentItemData.name}, CurrentWeight=${currentWeight.toFixed(2)}, AddedWeight=${addedWeight.toFixed(2)}, MaxWeight=${containerMaxWeight}`); if (currentWeight + addedWeight > containerMaxWeight + 0.001) { ui.notifications.warn(game.i18n.format("WEIGHTY_CONTAINERS.Notifications.CapacityExceeded", { containerName: containerItem.name })); capacityBlocked = true; console.log(`${MODULE_ID} | checkCapacityOnCreate: BLOCKED item ${currentItemData.name} due to capacity.`); } else { if (needsModification) { console.log(`${MODULE_ID} | checkCapacityOnCreate: Applying reduction TO CREATION DATA for ${currentItemData.name}. Reduced weight: ${reducedWeight}, Base: ${itemBaseWeight}`); currentItemData.system.weight = reducedWeight; if (!currentItemData.flags) currentItemData.flags = {}; if (!currentItemData.flags[MODULE_ID]) currentItemData.flags[MODULE_ID] = {}; currentItemData.flags[MODULE_ID][FLAG_ORIGINAL_WEIGHT] = itemBaseWeight; } else if (reductionPercent > 0 && itemBaseWeight > 0 && !(currentItemData.flags?.[MODULE_ID]?.[FLAG_ORIGINAL_WEIGHT])) { console.log(`${MODULE_ID} | checkCapacityOnCreate: Setting flag for already reduced/0 weight item ${currentItemData.name}. Base: ${itemBaseWeight}`); if (!currentItemData.flags) currentItemData.flags = {}; if (!currentItemData.flags[MODULE_ID]) currentItemData.flags[MODULE_ID] = {}; currentItemData.flags[MODULE_ID][FLAG_ORIGINAL_WEIGHT] = itemBaseWeight; } itemsToCreate.push(currentItemData); } }
    console.log(`${MODULE_ID} | checkCapacityOnCreate: Finished processing items. Blocked: ${capacityBlocked}, Items to create: ${itemsToCreate.length}`); console.log(`${MODULE_ID} | >>>>> CHECK CAPACITY ON CREATE END <<<<<`);
    if (capacityBlocked) { if (itemsToCreate.length === 0) { return []; } console.log(`${MODULE_ID} | checkCapacityOnCreate: Calling wrapped with allowed items.`); return wrapped(embeddedName, itemsToCreate, context); } else { console.log(`${MODULE_ID} | checkCapacityOnCreate: Calling wrapped with all items.`); return wrapped(embeddedName, itemsToCreate, context); }
}

/** LibWrapper: Combined handler for Item.update */
async function combinedItemUpdateHandler(wrapped, changes, context) {
    const item = this; if (!item.actor || context?.[INTERNAL_UPDATE_FLAG] || ['container', 'backpack'].includes(item.type)) { return wrapped(changes, context); } const actor = item.actor;
    console.log(`${MODULE_ID} | >>>>> COMBINED ITEM UPDATE HANDLER START | Item: ${item.name} (${item.id}) on Actor: ${actor.name} <<<<<`); console.log(`${MODULE_ID} | combinedItemUpdateHandler: Incoming changes:`, foundry.utils.deepClone(changes)); console.log(`${MODULE_ID} | combinedItemUpdateHandler: Context:`, context);
