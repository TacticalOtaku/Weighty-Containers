...
async function applyWeightReduction(item, containerItem, context = {}) {
    ...
    console.log(`${MODULE_ID} | Attempting applyWeightReduction for ${item.name}. Reduction: ${reductionPercent}%`);
    if (reductionPercent <= 0 || reductionPercent > 100) {
         console.log(`${MODULE_ID} | applyWeightReduction: Invalid reduction percent (${reductionPercent}). Skipping.`);
    ...
    console.log(`${MODULE_ID} | applyWeightReduction Calc: Original=${originalWeight}, Current=${currentWeight}, ExpectedReduced=${expectedReducedWeight}`);
    let needsUpdate = false;
    let needsFlag = false;
    if (getOriginalWeight(item) === null || Math.abs(getOriginalWeight(item) - originalWeight) > 0.001) { needsFlag = true; }
...
/**
 * Восстанавливает оригинальный вес предмета.
...
async function restoreOriginalWeight(item, context = {}) {
    ...
    console.log(`${MODULE_ID} | Attempting restoreOriginalWeight for ${item.name}. Flag value: ${originalWeight}`);
...
Hooks.on('renderItemSheet5e', (app, html, data) => {
    ...
    // *** УЛУЧШЕННАЯ ПРОВЕРКА ВМЕСТИМОСТИ ***
    const weightCapacityValue = item.system?.capacity?.weight?.value;
    // Проверяем, что значение существует, не пустое, и является конечным числом после преобразования
    ...
    // Детальное логирование для отладки этой проверки
    console.log(`${MODULE_ID} | Checking Item: ${item.name}`); // <-- Log item name
    console.log(`${MODULE_ID} |   > system.capacity.weight object:`, foundry.utils.deepClone(item.system?.capacity?.weight)); // <-- Log the object
    console.log(`${MODULE_ID} |   > system.capacity.weight.value: ${weightCapacityValue} (Type: ${typeof weightCapacityValue})`); // <-- Log value and type
    console.log(`${MODULE_ID} |   > Result of Number.isFinite(Number(weightCapacityValue)): ${Number.isFinite(Number(weightCapacityValue))}`); // <-- Log conversion result
    console.log(`${MODULE_ID} |   > Final hasWeightCapacity check result: ${hasWeightCapacity}`); // <-- Log final check

    if (!hasWeightCapacity) {
         console.log(`${MODULE_ID} | Item ${item.name} does not appear to have a valid finite number for weight capacity. Skipping UI.`); // <--- Log
        return;
    }
    // Если проверка прошла, продолжаем
    console.log(`${MODULE_ID} | Item ${item.name} IS a valid container with weight capacity defined. Value: ${weightCapacityValue}`); // <--- Лог
    ...
    ` : '';
    const controlsContainerHtml = `<div class="cwm-controls-container">${displayHtml}${buttonHtml}</div>`;
    console.log(`${MODULE_ID} | Generated HTML:`, controlsContainerHtml);

    // --- ЛОГИКА ВНЕДРЕНИЯ HTML ---
    const capacityHeaderText = game.i18n.localize("DND5E.Capacity");
    const capacityHeader = html.find(`legend:contains("${capacityHeaderText}"), h4:contains("${capacityHeaderText}")`);
    let targetElement = capacityHeader.parent(); // fieldset or div containing the section

    if (targetElement.length > 0) {
        console.log(`${MODULE_ID} | Found capacity section parent element (${targetElement.prop("tagName")}). Appending controls inside.`);
        html.find('.cwm-controls-container').remove(); // Remove old first
        targetElement.append(controlsContainerHtml);
    } else {
        console.warn(`${MODULE_ID} | Capacity section header not found. Trying details tab.`);
        targetElement = html.find('.tab[data-tab="details"]');
        if (targetElement.length > 0) {
            console.log(`${MODULE_ID} | Found details tab. Prepending controls.`);
            targetElement.find('.cwm-controls-container').remove();
            targetElement.prepend(controlsContainerHtml);
        } else { console.error(`${MODULE_ID} | Could not find any suitable place to insert UI controls!`); }
    }
    ...
    if (canConfigure) {
        ...
        if (buttonElement.length > 0) {
            console.log(`${MODULE_ID} | Adding click listener to configure button.`);
            buttonElement.off('click').on('click', async (event) => {
                event.preventDefault(); event.stopPropagation();
                console.log(`${MODULE_ID} | Configure button clicked!`);
                const currentItem = app.object;
                if (!currentItem) { console.error(`${MODULE_ID} | Cannot find item object on button click.`); return; }
                const currentVal = getReductionPercent(currentItem);
                new Dialog({
                    title: game.i18n.localize("WEIGHTY_CONTAINERS.Dialog.SetReductionTitle"),
                    content: `<form><div class="form-group"><label>${game.i18n.localize("WEIGHTY_CONTAINERS.Dialog.SetReductionLabel")}</label><div class="form-fields"><input type="number" name="reductionPercent" value="${currentVal}" min="0" max="100" step="1"/></div><p class="notes">${game.i18n.localize("WEIGHTY_CONTAINERS.Dialog.SetReductionHint")}</p></div></form>`,
                    buttons: {
                        save: { icon: '<i class="fas fa-save"></i>', label: game.i18n.localize("Save"), callback: async (html) => {
                            const itemToUpdate = app.object; if (!itemToUpdate) return;
                            const inputVal = html.find('input[name="reductionPercent"]').val(); const newPercentage = parseInt(inputVal, 10);
                            if (isNaN(newPercentage) || newPercentage < 0 || newPercentage > 100) { ui.notifications.warn(game.i18n.localize("WEIGHTY_CONTAINERS.Notifications.InvalidPercentage")); return false; }
                            console.log(`${MODULE_ID} | Dialog Save: Setting reduction for ${itemToUpdate.name} to ${newPercentage}%`);
                            await itemToUpdate.setFlag(MODULE_ID, FLAG_REDUCTION_PERCENT, newPercentage);
                            if (itemToUpdate.actor) {
                                console.log(`${MODULE_ID} | Dialog Save: Updating contained items weight for actor ${itemToUpdate.actor.name}.`);
                                await updateContainedItemsWeight(itemToUpdate.actor, itemToUpdate, newPercentage);
                            } else { console.log(`${MODULE_ID} | Dialog Save: Container item does not belong to an actor. Skipping contained items update.`); }
                        }},
                        cancel: { icon: '<i class="fas fa-times"></i>', label: game.i18n.localize("Cancel") }
                    }, default: 'save', render: html => { html.find('input[name="reductionPercent"]').focus().select(); }
                }).render(true);
            });
...
/**
 * Обновляет вес всех предметов внутри контейнера.
 * @param {Actor5e} actor Владелец
 * @param {Item5e} containerItem Контейнер
 * @param {number} newReductionPercent Новый процент
 */
async function updateContainedItemsWeight(actor, containerItem, newReductionPercent) {
    if (!actor || !containerItem) return;
    console.log(`${MODULE_ID} | updateContainedItemsWeight: Updating items in ${containerItem.name} for actor ${actor.name} to ${newReductionPercent}% reduction.`);
    const containedItems = actor.items.filter(i => i && i.system.container?.id === containerItem.id && !['container', 'backpack'].includes(i.type));
    if (!containedItems.length) { console.log(`${MODULE_ID} | updateContainedItemsWeight: No relevant items found inside ${containerItem.name}.`); return; }
    const updates = [];
    const itemsToRestore = [];
    const internalContext = { [INTERNAL_UPDATE_FLAG]: true };
    for (const item of containedItems) {
        if (!item) continue; const originalWeight = getOriginalWeight(item); const baseWeightForCalc = originalWeight ?? item.system.weight;
        if (baseWeightForCalc === 0) { if (newReductionPercent === 0 && originalWeight !== null) { await item.unsetFlag(MODULE_ID, FLAG_ORIGINAL_WEIGHT); } else if (newReductionPercent > 0 && originalWeight === null) { await item.setFlag(MODULE_ID, FLAG_ORIGINAL_WEIGHT, 0); } continue; }
        if (newReductionPercent > 0) {
            const reductionFactor = 1 - (newReductionPercent / 100); const expectedReducedWeight = Math.max(0, baseWeightForCalc * reductionFactor);
            if (originalWeight === null || Math.abs(originalWeight - baseWeightForCalc) > 0.001) { await item.setFlag(MODULE_ID, FLAG_ORIGINAL_WEIGHT, baseWeightForCalc); }
            if (Math.abs(item.system.weight - expectedReducedWeight) > 0.001) { console.log(`${MODULE_ID} | updateContainedItemsWeight: Queuing weight update for ${item.name}. Expected: ${expectedReducedWeight}, Actual: ${item.system.weight}`); updates.push({ _id: item.id, 'system.weight': expectedReducedWeight }); }
        } else { if (originalWeight !== null) { console.log(`${MODULE_ID} | updateContainedItemsWeight: Reduction is 0%, queuing restore for ${item.name}.`); itemsToRestore.push(item); } }
    }
    if (updates.length > 0) {
        ...
        try { await actor.updateEmbeddedDocuments("Item", updates, internalContext); } catch (err) { console.error(`${MODULE_ID} | Error applying batch updates in updateContainedItemsWeight for actor ${actor.name}:`, err); }
    } else { console.log(`${MODULE_ID} | updateContainedItemsWeight: No batch weight updates needed for items in ${containerItem.name}.`); }
    if (itemsToRestore.length > 0) {
        console.log(`${MODULE_ID} | updateContainedItemsWeight: Restoring original weight for ${itemsToRestore.length} items.`);
        for (const itemToRestore of itemsToRestore) { await restoreOriginalWeight(itemToRestore, internalContext); }
    }
}
...
async function checkCapacityOnCreate(wrapped, embeddedName, data, context) {
    ...
    for (const itemData of data) {
        ...
        } else { if (reductionPercent > 0 && itemBaseWeight > 0) { console.log(`${MODULE_ID} | checkCapacityOnCreate: Applying reduction to creation data for ${itemData.name}. Reduced weight: ${reducedWeight}`); itemData.system.weight = reducedWeight; if (!itemData.flags) itemData.flags = {}; if (!itemData.flags[MODULE_ID]) itemData.flags[MODULE_ID] = {}; itemData.flags[MODULE_ID][FLAG_ORIGINAL_WEIGHT] = itemBaseWeight; } itemsToCreate.push(itemData); }
    }
    ...
    else { console.log(`${MODULE_ID} | checkCapacityOnCreate: All items allowed.`); return wrapped(embeddedName, data, context); }
}

/** LibWrapper: Combined handler for Item.update */
async function combinedItemUpdateHandler(wrapped, changes, context) {
    // console.log(`${MODULE_ID} | combinedItemUpdateHandler fired for item: ${this?.name}, changes:`, foundry.utils.deepClone(changes)); // Reduce console spam
    const item = this;
    ...
    if (isQuantityIncrease && containerId) {
        const containerItem = actor.items.get(containerId);
        const weightCapacityValue = containerItem?.system?.capacity?.weight?.value; const containerHasWeightCapacity = weightCapacityValue !== null && weightCapacityValue !== undefined && weightCapacityValue !== "" && Number.isFinite(Number(weightCapacityValue));
        if (containerHasWeightCapacity) {
            const containerMaxWeight = Number(weightCapacityValue) ?? 0; const currentWeightOfOthers = calculateContainerContentsWeight(containerItem, actor) - (item.system.weight * item.system.quantity); const newWeightOfThisItem = item.system.weight * quantityChange;
            if (currentWeightOfOthers + newWeightOfThisItem > containerMaxWeight + 0.001) {
                ui.notifications.warn(game.i18n.format("WEIGHTY_CONTAINERS.Notifications.CapacityExceeded", { containerName: containerItem.name })); console.log(`${MODULE_ID} | combinedItemUpdateHandler: BLOCKED quantity update for ${item.name} due to capacity.`);
                delete changes.system.quantity; if (foundry.utils.isEmpty(changes.system)) delete changes.system; if (foundry.utils.isEmpty(changes)) { return item; }
            }
        }
    }
    // --- 2. WRAPPED CALL ---
    ...
    // console.log(`${MODULE_ID} | combinedItemUpdateHandler: Calling wrapped update for ${item.name}`);
    const updatedItem = await wrapped(changes, context); // Original update happens
    // console.log(`${MODULE_ID} | combinedItemUpdateHandler: Wrapped update finished for ${updatedItem?.name}.`);
    // --- 3. POST-UPDATE LOGIC ---
    ...
    if (containerChanged && previousContainerId !== newContainerId) {
        ...
        if (wasInReducingContainer && !isInReducingContainer) { await restoreOriginalWeight(updatedItem, internalContext); }
        else if (!wasInReducingContainer && isInReducingContainer) { await applyWeightReduction(updatedItem, newContainer, internalContext); }
        else if (wasInReducingContainer && isInReducingContainer) { await applyWeightReduction(updatedItem, newContainer, internalContext); }
    }
    // --- 3.2 External Weight Change ---
    else if (weightChanged && isInReducingContainer && previousContainerId === newContainerId) {
         console.log(`${MODULE_ID} | combinedItemUpdateHandler: External weight change detected for ${updatedItem.name} while in reducing container ${newContainer.name}.`);
         const newOriginalWeight = updatedItem.system.weight; const currentFlagWeight = getOriginalWeight(updatedItem);
         if (currentFlagWeight === null || Math.abs(currentFlagWeight - newOriginalWeight) > 0.001) {
            ...
            const reductionFactor = 1 - (getReductionPercent(newContainer) / 100); const newReducedWeight = Math.max(0, newOriginalWeight * reductionFactor);
            if (Math.abs(updatedItem.system.weight - newReducedWeight) > 0.001) { await updatedItem.update({'system.weight': newReducedWeight}, internalContext); }
         } else { console.log(`${MODULE_ID} | combinedItemUpdateHandler: External weight change matches existing flag for ${updatedItem.name}. No action needed.`); }
    }
    // --- 3.3 Flag Consistency Check ---
    ...
    // console.log(`${MODULE_ID} | combinedItemUpdateHandler: Finished post-update logic for ${updatedItem.name}.`);
    return updatedItem;
}


// --- Fallback Функции (с обновленными проверками вместимости) ---
function onItemPreCreate(item, data, options, userId) {
    // console.log(`${MODULE_ID} | Fallback: onItemPreCreate for ${data.name}`);
    if (game.userId !== userId) return true; const actor = item.parent; if (!actor || ['container', 'backpack'].includes(data.type)) return true; const containerId = data.system?.container; if (!containerId) return true; const containerItem = actor.items.get(containerId);
    const weightCapacityValue = containerItem?.system?.capacity?.weight?.value; const containerHasWeightCapacity = weightCapacityValue !== null && weightCapacityValue !== undefined && weightCapacityValue !== "" && Number.isFinite(Number(weightCapacityValue));
    if (!containerHasWeightCapacity) return true;
    const containerMaxWeight = Number(weightCapacityValue) ?? 0; const currentWeight = calculateContainerContentsWeight(containerItem, actor); let itemBaseWeight = data.system?.weight ?? 0; const quantity = data.system?.quantity ?? 1; const reductionPercent = getReductionPercent(containerItem); let reducedWeight = itemBaseWeight;
    if (reductionPercent > 0 && itemBaseWeight > 0) { reducedWeight = Math.max(0, itemBaseWeight * (1 - reductionPercent / 100)); } const addedWeight = reducedWeight * quantity;
    if (currentWeight + addedWeight > containerMaxWeight + 0.001) { ui.notifications.warn(game.i18n.format("WEIGHTY_CONTAINERS.Notifications.CapacityExceeded", { containerName: containerItem.name })); return false; }
    if (reductionPercent > 0 && itemBaseWeight > 0) { data.system.weight = reducedWeight; if (!data.flags) data.flags = {}; if (!data.flags[MODULE_ID]) data.flags[MODULE_ID] = {}; data.flags[MODULE_ID][FLAG_ORIGINAL_WEIGHT] = itemBaseWeight; } return true;
}
function onItemPreUpdate(item, changes, options, userId) {
    // console.log(`${MODULE_ID} | Fallback: onItemPreUpdate for ${item.name}, changes:`, foundry.utils.deepClone(changes));
    if (game.userId !== userId || options?.[INTERNAL_UPDATE_FLAG]) return true; const actor = item.actor; if (!actor || ['container', 'backpack'].includes(item.type)) return true; const quantityChange = changes.system?.quantity; const isQuantityIncrease = typeof quantityChange === 'number' && quantityChange > item.system.quantity; const containerId = item.system?.container?.id; if (!isQuantityIncrease || !containerId) return true; const containerItem = actor.items.get(containerId);
    const weightCapacityValue = containerItem?.system?.capacity?.weight?.value; const containerHasWeightCapacity = weightCapacityValue !== null && weightCapacityValue !== undefined && weightCapacityValue !== "" && Number.isFinite(Number(weightCapacityValue));
    if (!containerHasWeightCapacity) return true;
    const containerMaxWeight = Number(weightCapacityValue) ?? 0; const currentWeightOfOthers = calculateContainerContentsWeight(containerItem, actor) - (item.system.weight * item.system.quantity); const newWeightOfThisItem = item.system.weight * quantityChange;
    if (currentWeightOfOthers + newWeightOfThisItem > containerMaxWeight + 0.001) { ui.notifications.warn(game.i18n.format("WEIGHTY_CONTAINERS.Notifications.CapacityExceeded", { containerName: containerItem.name })); delete changes.system.quantity; if (foundry.utils.isEmpty(changes.system)) delete changes.system; if (foundry.utils.isEmpty(changes)) return false; return true; } return true;
}
async function onItemUpdate(item, changes, options, userId) {
    // console.log(`${MODULE_ID} | Fallback: onItemUpdate for ${item.name}, changes:`, foundry.utils.deepClone(changes));
    if (game.userId !== userId || options?.[INTERNAL_UPDATE_FLAG] ) return; const actor = item.actor; if (!actor || ['container', 'backpack'].includes(item.type)) return; const internalContext = { [INTERNAL_UPDATE_FLAG]: true }; const newContainerId = item.system.container?.id; const newContainer = newContainerId ? actor.items.get(newContainerId) : null;
    const newWeightCapacityValue = newContainer?.system?.capacity?.weight?.value; const newContainerHasWeightCap = newWeightCapacityValue !== null && newWeightCapacityValue !== undefined && newWeightCapacityValue !== "" && Number.isFinite(Number(newWeightCapacityValue));
    const isInReducingContainer = newContainer && getReductionPercent(newContainer) > 0 && newContainerHasWeightCap; const hadOriginalWeightFlag = getOriginalWeight(item) !== null;
    if (changes.system && 'container' in changes.system) {
        const previousContainerId = changes.system.container; if (previousContainerId !== newContainerId) { const previousContainer = previousContainerId ? actor.items.get(previousContainerId) : null; const prevWeightCapacityValue = previousContainer?.system?.capacity?.weight?.value; const prevContainerHasWeightCap = prevWeightCapacityValue !== null && prevWeightCapacityValue !== undefined && prevWeightCapacityValue !== "" && Number.isFinite(Number(prevWeightCapacityValue)); const wasInReducingContainer = previousContainer && getReductionPercent(previousContainer) > 0 && prevContainerHasWeightCap;
             if (wasInReducingContainer && !isInReducingContainer) { await restoreOriginalWeight(item, internalContext); } else if (!wasInReducingContainer && isInReducingContainer) { await applyWeightReduction(item, newContainer, internalContext); } else if (wasInReducingContainer && isInReducingContainer) { await applyWeightReduction(item, newContainer, internalContext); } }
...
// --- Ready Hook (с обновленными проверками вместимости) ---
Hooks.on('ready', async () => {
    console.log(`${MODULE_ID} | Ready Hook Fired. Checking item weights inside containers.`);
    const actorsToCheck = new Set(); game.users.filter(u => u.character && u.active).forEach(u => actorsToCheck.add(u.character)); game.scenes.filter(s => s.active).flatMap(s => s.tokens).filter(t => t.actor).forEach(t => actorsToCheck.add(t.actor));
    for (const actor of actorsToCheck) {
        if (!actor?.items) continue; console.log(`${MODULE_ID} | Ready Check: Processing actor ${actor.name}`); const updates = []; const itemsToProcess = actor.items.contents; const internalContext = { [INTERNAL_UPDATE_FLAG]: true };
        for (const item of itemsToProcess) {
             if (!item || ['container', 'backpack'].includes(item.type)) continue; const containerId = item.system.container?.id; const originalWeight = getOriginalWeight(item);
             if (!containerId) { if (originalWeight !== null) { console.log(`${MODULE_ID} | Ready Check: Restoring weight for ${item.name} (not in container but has flag). Actor: ${actor.name}`); await restoreOriginalWeight(item, internalContext); } continue; }
             const container = actor.items.get(containerId);
             const weightCapacityValue = container?.system?.capacity?.weight?.value; const containerHasWeightCapacity = weightCapacityValue !== null && weightCapacityValue !== undefined && weightCapacityValue !== "" && Number.isFinite(Number(weightCapacityValue));
             if (!containerHasWeightCapacity) { if (originalWeight !== null) { console.log(`${MODULE_ID} | Ready Check: Restoring weight for ${item.name} (invalid container but has flag). Actor: ${actor.name}`); await restoreOriginalWeight(item, internalContext); } continue; }
             const reductionPercent = getReductionPercent(container);
             if (reductionPercent > 0) {
                const baseWeight = originalWeight ?? item.system.weight; if (baseWeight === 0) { if(originalWeight === null) await item.setFlag(MODULE_ID, FLAG_ORIGINAL_WEIGHT, 0); continue; }
                const reductionFactor = 1 - (reductionPercent / 100); const expectedReducedWeight = Math.max(0, baseWeight * reductionFactor); let needsFlagUpdate = false; let needsWeightUpdate = false;
                if (originalWeight === null || Math.abs(originalWeight - baseWeight) > 0.001) { needsFlagUpdate = true; } if (Math.abs(item.system.weight - expectedReducedWeight) > 0.001) { needsWeightUpdate = true; }
                if (needsFlagUpdate) { await item.setFlag(MODULE_ID, FLAG_ORIGINAL_WEIGHT, baseWeight); } if (needsWeightUpdate) { console.log(`${MODULE_ID} | Ready Check: Correcting weight for ${item.name} in ${container.name}. Expected: ${expectedReducedWeight}, Actual: ${item.system.weight}. Actor: ${actor.name}`); updates.push({ _id: item.id, 'system.weight': expectedReducedWeight }); }
